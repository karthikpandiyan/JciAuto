...
namespace JCI.CAM.AutoTaggingAppWeb.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            foreach (char c in "Tata@1234".ToCharArray()) passWord.AppendChar(c);
            using (var clientContext = spContext.CreateUserClientContextForSPHost())
            {
                //var cred = new SharePointOnlineCredentials("karthikeyan@jcipoc.onmicrosoft.com", passWord);
                //clientContext.Credentials = cred;
                try
                {
                    ////Event Receiver
                    //List _list = clientContext.Web.Lists.GetByTitle("Jci");
                    //EventReceiverDefinitionCreationInformation _rec = CreateEventReciever("ItemAdding2", EventReceiverType.ItemAdding);
                    //bool exist = AddEventReceiver(clientContext, _list, _rec);
                    //ViewBag.Message = exist + " - Event Receiver registered -success!!!";
...
//
    ...
    foreach (char c in "Tata@1234".ToCharArray()) passWord.AppendChar(c);
            using (var clientContext = spContext.CreateUserClientContextForSPHost())
            {
                ...
                {
                    //Event Receiver
                    List _list = clientContext.Web.Lists.GetByTitle("JciAuto");
                    EventReceiverDefinitionCreationInformation _rec = CreateEventReciever("ItemAdding5", EventReceiverType.ItemAdding);
                    bool exist=   AddEventReceiver(clientContext, _list, _rec);
                    ViewBag.Message = exist+ " - Event Receiver registered -success!!!";
                }
                catch (Exception ex)
        ...
        public  EventReceiverDefinitionCreationInformation CreateEventReciever(string receiverName, EventReceiverType type)
        {
            ...
            _rer.ReceiverUrl = "https://jciautotag.azurewebsites.net/autotag/services/autotaggingservice.svc";
